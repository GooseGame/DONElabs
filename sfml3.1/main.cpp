...
void update(const sf::Vector2f &mousePosition, sf::ConvexShape &pointer, float &deltaTime)
{
    ...
    float maximRotation = maximAngle * deltaTime;
    float rotation = std::abs((angle - pointer.getRotation()));
    if (angle < pointer.getRotation())
    {
        if ((angle + 180) < pointer.getRotation())
        {
            pointer.setRotation(pointer.getRotation() + std::min(maximRotation, rotation));
        }
        else
        {
            pointer.setRotation(pointer.getRotation() - std::min(maximRotation, rotation));
